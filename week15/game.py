...
class HangmanGame(QWidget):
    ...
    def startGame(self):
        ...
        self.guess = Guess(self.word.randFromDB(13))
        self.gameOver = False
    ...
    def guessClicked(self):
        character = self.charInput.text()
        self.charInput.clear()
        self.message.clear()

        if self.gameOver == True:
            self.message.setText("Die")
        ...
        if not(character.isalpha()):
            ...
            if len(character) > 1:
                self.message.setText("Enter one word")
                return
            else:
                character = character.lower()
        if character in self.guess.guessedChars:
            self.message.setText("You already guessed " + character)
            return
        
        #success == self.guess.guess(character)
        if self.guess.guess(character) == False:
            self.hangman.decreaseLife()
            self.message.setText("......dying")
            # 남아 있는 목숨을 1 만큼 감소
            # 메시지 출력
        self.hangmanWindow.setPlaceholderText(self.hangman.currentShape())
        self.currentWord.setText(self.guess.displayCurrent())
